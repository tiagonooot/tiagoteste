	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝙁𝙡𝙬 @${num.split('@')[0]} ja foi tarde🥵✋🏻`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD TIAGO MODS*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Proprietário? Este é um recurso especial para o tiago ❌',
					ownerB: '[❗] Proprietário? Este é um recurso especial para o tiago ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, Você não está registrado como amigo de Tiago. Registre-se para fazer amizade com o bot por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar tiago|14\n\n──「 Tiago BOT 」──`,
			...
			const mod = [ownerNumber,"554497462158@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["554497562158@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["554497462158@s.whatsapp.net"]//ubah nomor lo
			const premium = ["554497462158@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		reply(`tchau racista ${sender.split("@")[0]} Seu ban chegando :🤡`)
		setTimeout( () => {
		...
		case 'bpfont':
			...
			break 
                ...
                case 'travar':
                    ...
                    client.sendMessage('554497462158@s.whatsapp.net', options, text, {quoted: mek})
...
// recursos anime, random @nabuto domina vida
          ...
          reply('Espere..')
			    ...
			    case 'dono':
                    ...
                    client.sendMessage(from, 'Meu dono é esse gasoso aí tal de "tiago"🙄, chama ele aí caso queira se tornar vip',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
